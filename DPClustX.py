...
class ClusteringExplainer:
    ...
    def get_domains(self, domains):
        if domains is None:
            warnings.warn("Domains not provided. Using domains of input dataset, which may result in privacy leakage.")
            domains = {
    ...
    def top_single_cluster_exp_attrs(self, cluster_id, eps_single, k, exclude, private=True):

        if exclude is None or cluster_id not in exclude:
            exclude = {cluster_id: []}

        scores = {A: self.score_single(cluster_id, A) for A in self.attributes if A not in exclude[cluster_id]}
    ...
    def explain_admin(self, eps_candlist, eps_topcomb, eps_hist, exclude):
        ...
        for cluster_id in self.clusters_ids:
            single_exp = self.top_single_cluster_exp_attrs(
                cluster_id, eps_single, self.num_candidates, exclude, private=priv_candlist
            )
        ...
        global_explanation = self.gen_noisy_hists(top_comb, eps_hist or 0, priv_hist)
        self.global_explanation = global_explanation
        self.show_explanation()

        return global_explanation

    def explain_user(self, eps_candlist, eps_topcomb, eps_hist, exclude):
        ...
        for cluster_id in self.clusters_ids:
            single_exp = self.top_single_cluster_exp_attrs(
                cluster_id, eps_single, self.num_candidates, exclude, private=True
            )
            # print(cluster_id, single_exp)
            explanation_candidates.append(single_exp)
